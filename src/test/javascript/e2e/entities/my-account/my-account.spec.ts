...
describe('MyAccount e2e test', () => {
    ...
    it('should create and save MyAccounts', async () => {
        ...
        expect(await myAccountUpdatePage.getBalanceInput()).to.eq('5');
        await myAccountUpdatePage.save();
        expect(await myAccountUpdatePage.getSaveButton().isPresent()).to.be.false;

        expect(await myAccountComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1);
    });
