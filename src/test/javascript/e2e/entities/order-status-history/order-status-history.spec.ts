...
describe('OrderStatusHistory e2e test', () => {
    ...
    it('should create and save OrderStatusHistories', async () => {
        ...
        expect(await orderStatusHistoryUpdatePage.getEffectiveDateInput()).to.contain('2001-01-01T02:30');
        await orderStatusHistoryUpdatePage.save();
        expect(await orderStatusHistoryUpdatePage.getSaveButton().isPresent()).to.be.false;

        expect(await orderStatusHistoryComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1);
    });
