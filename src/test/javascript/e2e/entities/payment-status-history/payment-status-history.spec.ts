...
describe('PaymentStatusHistory e2e test', () => {
    ...
    it('should create and save PaymentStatusHistories', async () => {
        ...
        expect(await paymentStatusHistoryUpdatePage.getEffectiveDateInput()).to.contain('2001-01-01T02:30');
        await paymentStatusHistoryUpdatePage.save();
        expect(await paymentStatusHistoryUpdatePage.getSaveButton().isPresent()).to.be.false;

        expect(await paymentStatusHistoryComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1);
    });
