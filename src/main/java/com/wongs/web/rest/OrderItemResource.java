...
import com.wongs.service.OrderItemService;
...
/**
 * REST controller for managing {@link com.wongs.domain.OrderItem}.
 */
@RestController
@RequestMapping("/api")
public class OrderItemResource {
    ...
    public ResponseEntity<List<OrderItemDTO>> getAllOrderItems(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("productitem-is-null".equals(filter)) {
            ...
            return new ResponseEntity<>(orderItemService.findAllWhereProductItemIsNull(),
                    HttpStatus.OK);
        }
    ...
    public ResponseEntity<List<OrderItemDTO>> searchOrderItems(@RequestParam String query, Pageable pageable) {
        ...
        Page<OrderItemDTO> page = orderItemService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
