...
export class ProductHistoryComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.productHistoryService.search({
                query: this.currentSearch,
                page: this.page,
                size: this.itemsPerPage,
                sort: this.sort()
            }).subscribe(
                (res: HttpResponse<ProductHistory[]>) => this.onSuccess(res.body, res.headers),
                (res: HttpErrorResponse) => this.onError(res.message)
            );
            return;
        }
        this.productHistoryService.query({
            page: this.page,
            size: this.itemsPerPage,
            sort: this.sort()
        }).subscribe(
    ...
    trackId(index: number, item: ProductHistory) {
        return item.id;
    }
    registerChangeInProductHistories() {
        this.eventSubscriber = this.eventManager.subscribe('productHistoryListModification', (response) => this.reset());
    }
