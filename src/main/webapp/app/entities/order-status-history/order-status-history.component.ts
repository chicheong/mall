...
export class OrderStatusHistoryComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.orderStatusHistoryService
                ...
                .subscribe(
                    (res: HttpResponse<IOrderStatusHistory[]>) => this.paginateOrderStatusHistories(res.body, res.headers),
    ...
    trackId(index: number, item: IOrderStatusHistory) {
        return item.id;
    }

    registerChangeInOrderStatusHistories() {
        this.eventSubscriber = this.eventManager.subscribe('orderStatusHistoryListModification', response => this.reset());
    }
    ...
    protected paginateOrderStatusHistories(data: IOrderStatusHistory[], headers: HttpHeaders) {
