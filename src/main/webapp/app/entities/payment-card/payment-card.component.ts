...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.paymentCardService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<PaymentCard[]>) => this.onSuccess(res.body, res.headers),
                    (res: HttpErrorResponse) => this.onError(res.message)
                );
            return;
        }
        this.paymentCardService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<PaymentCard[]>) => this.onSuccess(res.body, res.headers),
    ...
    trackId(index: number, item: PaymentCard) {
        return item.id;
    }
