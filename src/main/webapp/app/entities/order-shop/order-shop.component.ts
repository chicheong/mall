...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.orderShopService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<OrderShop[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/order-shop'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: OrderShop) {
        return item.id;
    }
    registerChangeInOrderShops() {
        this.eventSubscriber = this.eventManager.subscribe('orderShopListModification', (response) => this.loadAll());
    }
