...
export class ShippingStatusHistoryComponent implements OnInit, OnDestroy {
    ...
    shippingStatusHistories: ShippingStatusHistory[];
    currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.shippingStatusHistoryService.search({
                query: this.currentSearch,
                page: this.page,
                size: this.itemsPerPage,
                sort: this.sort()
            }).subscribe(
                (res: HttpResponse<ShippingStatusHistory[]>) => this.onSuccess(res.body, res.headers),
                (res: HttpErrorResponse) => this.onError(res.message)
            );
            return;
        }
        this.shippingStatusHistoryService.query({
            page: this.page,
            size: this.itemsPerPage,
            sort: this.sort()
        }).subscribe(
    ...
    trackId(index: number, item: ShippingStatusHistory) {
        return item.id;
    }
