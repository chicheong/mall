...
@Component({
    selector: 'jhi-payment-credit-card',
...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.paymentCreditCardService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<PaymentCreditCard[]>) => this.onSuccess(res.body, res.headers),
                    (res: HttpErrorResponse) => this.onError(res.message)
                );
            return;
        }
        this.paymentCreditCardService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<PaymentCreditCard[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/payment-credit-card'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: PaymentCreditCard) {
        return item.id;
    }
    registerChangeInPaymentCreditCards() {
        this.eventSubscriber = this.eventManager.subscribe('paymentCreditCardListModification', (response) => this.loadAll());
    }
