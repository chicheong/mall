...
export class MyStateResolve implements Resolve<IMyState> {
  constructor(private service: MyStateService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IMyState> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((myState: HttpResponse<MyState>) => {
          if (myState.body) {
            return of(myState.body);
          } else {
