...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.shippingTypeService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<ShippingType[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/shipping-type'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: ShippingType) {
        return item.id;
    }
    registerChangeInShippingTypes() {
        this.eventSubscriber = this.eventManager.subscribe('shippingTypeListModification', (response) => this.loadAll());
    }
